...
text CameraDevice::format_display_string_type(SDK::CrDisplayStringType type) {
    ...
    case SCRSDK::CrDisplayStringType_CreativeLook_Name_Display:
        ...
        break;
    case SCRSDK::CrDisplayStringType_Reserved9:
        ts << "reserved";
        break;
...
void CameraDevice::setMonitoringDeriverySetting()
{
    ...
    tout << "Please enter the Port Number\nset defalut if input 0." << std::endl;
    tout << "intput > ";
    text portNumStr;
    std::getline(tin, portNumStr);
    ...
    *strLen = ipLen + 1; // String length, include Null-terminate
    ...
    CrInt32u downTime = std::stoi(downTimeStr);
    CrInt32u portNum = std::stoi(portNumStr);

    setting->downTime = downTime;
    setting->videoPort = portNum;
...
void CameraDevice::check_monitoringstatus()
{
    ...
    if (prop_list && 0 < nprop) {
        ...
        if (getCode == prop.GetCode()) {
            tout << "Monitoring Is Delivering : " << format_monitoring_is_delivery(prop.GetCurrentValue()) << std::endl;
        }
        SDK::ReleaseDeviceProperties(m_device_handle, prop_list);
    }
}
...
void CameraDevice::startMonitoring()
{
    ...
    SDK::CrError err = SDK::ControlMonitoring(m_device_handle, SDK::CrMonitoringOpertation_Start);
    if (CR_SUCCEEDED(err)) {
        tout << "Request OK \n";
    }
    else {
...
void CameraDevice::stopMonitoring()
{
    ...
    SDK::CrError err = SDK::ControlMonitoring(m_device_handle, SDK::CrMonitoringOpertation_Stop);
    if (CR_SUCCEEDED(err)) {
        tout << "Request OK \n";
    }
    else {
